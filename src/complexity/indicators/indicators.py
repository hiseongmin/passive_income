...
Implements 6 complexity indicators as defined in docs/complexity.md:
1. MA Separation
...
def calculate_volume_price_alignment(
    df: pd.DataFrame,
    window: int = 20,
    normalize_window: int = 100,
) -> pd.Series:
    """
    Calculate Volume-Price Alignment indicator.
    ...
    Returns:
        Series with normalized volume-price alignment (0-1, higher = better alignment)
    """
    close = df["close"]
    volume = df["volume"]

    # Calculate price change direction (1 for up, -1 for down)
    price_change = close.diff()

    # Calculate rolling correlation between |price_change| and volume
    # High volume with large price moves = good alignment
    abs_price_change = price_change.abs()

    # Rolling correlation
    correlation = abs_price_change.rolling(window=window).corr(volume)

    # Also consider: volume efficiency (volume that results in directional movement)
    # High volume but no price change = low alignment
    net_price_change = (close - close.shift(window)).abs()
    total_volume = volume.rolling(window=window).sum()
    volume_efficiency = net_price_change / (total_volume + 1e-8)

    # Normalize volume efficiency
    ve_min = volume_efficiency.rolling(window=normalize_window).min()
    ve_max = volume_efficiency.rolling(window=normalize_window).max()
    ve_norm = (volume_efficiency - ve_min) / (ve_max - ve_min + 1e-8)

    # Combine correlation and volume efficiency
    # Shift correlation from [-1, 1] to [0, 1]
    correlation_norm = (correlation + 1) / 2

    # Average of both metrics
    alignment = (correlation_norm.fillna(0.5) + ve_norm.fillna(0)) / 2

    return alignment.clip(0, 1).fillna(0)
...
def calculate_complexity_score(
    ...
    Args:
        ...
        weights: Dict with indicator weights (default: equal ~0.167 each)
        ...
        volume_window: Volume-price alignment window

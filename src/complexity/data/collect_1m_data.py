...
def fetch_klines(
    ...
    Returns:
        List of kline data
    """
    url = "https://api.binance.us/api/v3/klines"  # Use Binance.US (Binance.com blocked)
...
def collect_historical_data(
    ...
    end_date: str = None,
    save_path: Path = None,
    chunk_size: int = 100,  # Save every N requests
) -> pd.DataFrame:
    """
    Collect historical kline data from Binance with chunk-based saving.
    ...
    Args:
        ...
        chunk_size: Save to disk every N requests
    ...
    if save_path and save_path.exists():
        ...
        start_ms = int(last_time.timestamp() * 1000) + 60000  # +1 minute
    ...
    while current_start < end_ms:
        try:
            ...
            # Save chunk to disk
            if request_count % chunk_size == 0 and save_path and chunk_data:
                ...
                if first_write:
                    ...
                    chunk_df.to_csv(save_path, mode='a', header=False, index=False)

                print(f"  [Saved {len(chunk_df):,} rows to disk]")
    ...
    # Save remaining data
    if chunk_data and save_path:
        chunk_df = pd.concat(chunk_data, ignore_index=True)
        if first_write:
            chunk_df.to_csv(save_path, index=False)
        else:
            chunk_df.to_csv(save_path, mode='a', header=False, index=False)
        print(f"  [Saved final {len(chunk_df):,} rows]")

    # Load and return complete data
    if save_path and save_path.exists():
        df = pd.read_csv(save_path)
        df["open_time"] = pd.to_datetime(df["open_time"])
        df = df.drop_duplicates(subset=["open_time"])
        df = df.sort_values("open_time").reset_index(drop=True)
        df.to_csv(save_path, index=False)  # Save deduplicated version
